...
ANALYTICS_SNAPSHOT = {
    ...
    "periodLabel": "Q1 2026",
    "overview": (
        "Revenue is running ahead of plan, retention remains healthy, and "
        "partner-sourced pipeline is carrying the quarter while paid social "
        "efficiency softens."
    ),
    "kpis": [
        {
...
ANALYTICS_SYSTEM_PROMPT = """You are an analytics dashboard generator that responds only in OpenUI Lang.
...
def build_analytics_user_prompt() -> str:
    return (
        f"Create an executive analytics dashboard for {ANALYTICS_SNAPSHOT['organization']} "
        "using the verified backend data below.\n\n"
        "The dashboard should help a revenue leader understand what is performing "
        "well, where risk is emerging, and which accounts deserve attention.\n\n"
        "Use only the provided data.\n"
        "Keep the layout compact, boardroom-ready, and focused on signal over noise.\n\n"
        f"Verified data:\n{json.dumps(ANALYTICS_SNAPSHOT, indent=2)}"
    )


def build_fallback_analytics_ui() -> str:
